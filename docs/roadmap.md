...
- 当前模型（2026-09-16 换型，换型原因见下条）：画像抽取用 `deepseek-ai/DeepSeek-V4-Flash`（短结构化输出实测 ~1.3s），对话决策与推荐生成用 `zai-org/GLM-5.2`（~800 token 推荐实测 15.3s）。它们只是开发期实现，不代表最终自研模型选型。
- 换型背景：原 `Qwen/Qwen3-32B` 在 SiliconFlow 上存在 28s~185s 的排队尖峰（同一条 8 token 请求实测最好 1s、最差 185s），是"回复太慢"的主因；旧模型下三路模型调用的排队尖峰比生成耗时本身更影响体验。换型后实测各路径无尖峰，普通轮次约 5s、完整推荐轮次约 20s。
...
- 知识负载预算已落地（2026-09-16）：`findByStrategyIds` 的静默截断早已修复（分页取回 + 上限 200 + 截断量记入 `model_call_logs.knowledgeDropped`），这次补上真正的负载预算——`RecommendationService.trimToBudget` 在拼装完成后按策略整组核算：逐策略累计字符、超限的策略整组丢弃（不拆散一个策略的 chunk，定义与步骤分离的"半截资料"比没有更坏），第一个策略组无条件保留（宁可超预算也不让模型在空上下文里编造），被裁数继续记入 `knowledgeDropped`。预算起步 12000 字符（≈ 填厚前典型负载 4200 字符的 3 倍），以字符计而非 token：中文语料下 1 字符≈1 token 量级，不引入 tokenizer 即可充当保守近似；200 点收集上限保留为第一道兜底。
...
3. ~~定推荐链路的超时对策~~ **超时对策已落地**（公网部署阶段）：`readTimeout` 90→60 秒、`max_tokens` 1400→1200（**2026-09-16 调至 1600**：evidence 引用规则使典型输出增至约 1384 token，1200 反成截断源）、关闭思维链，抖动时快速失败走兜底，不挂过 Cloudflare 免费隧道 100 秒上限。**其遗留的知识负载预算也已落地**（2026-09-16）：`RecommendationService.trimToBudget` 按策略整组核算、超限整组丢弃、首组保底，被裁数记入 `knowledgeDropped`，预算 12000 字符起步（详见《当前方案的临时性质》）。这一项不再阻塞前两项。
